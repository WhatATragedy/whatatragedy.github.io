...
To do this we change the /etc/dhcpd/conf file, write the following command to open a text editor. Then we need to add the interface to the end of the file and give it a a static IP.
```bash
...
First let's set up the wireguard server, this is luckily quite simple. Choose your favourite cloud and spin up a VM (AWS, Azure, GCP). I used EC2 on AWS to spin up an Ubuntu server. This is a a Debian based distro and therefore if you want to follow along, it's useful to also spin up a Debian distro VM. To get wireguard installed, there is guide online. [Here](https://www.wireguard.com/install/).
...
AllowedIps = 10.0.0.2/32 # this will be hte IP address assigned to the client with this public key.
```
...
Now those changes to iptables is made, let's make sure they are presistent over boots
```bash
...
Let's create boiler plate config and add our keys
...
Here, the interface block assigns your Pi an IP address in the tunnel, which we defined on the server. The Peer block defines where our Pi goes to estbalish the tunnel. This is why we need to provide it with the servers public IP address and the servers public key.
...
You should now have a fully functioning Raspberry Pi VPN Access Gateway. Just to ensure it worked, add a device to the WiFi access point and naivgate to https://ifconfig.co/ or curl ifconfig.co and you should see your server's IP Address.
